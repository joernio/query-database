...
class UserInputIntoDangerousFunctionsTests  extends GhidraQueryTestSuite {
  ...
  "getenvToStrcpy query" when {
    ...
    "executed on CPG for binary with call to `strcpy`, and call to `getenv`, but no dataflow between them" should {
      "return an empty set of matched method names for a x86_64 binary" in {
        buildCpgForBin("buf2_neg.exe")
        val results = methodNamesForMatchedPoints(query)
        results shouldBe Set()
      }

      "return an empty set of matched method names for a MIPS binary" in {
        buildCpgForBin("buf2_neg.mips.exe")
        val results = methodNamesForMatchedPoints(query)
        results shouldBe Set()
      }
    }
    ...
    "executed on CPG for binary with dataflow between `getenv` return value and `strcpy` source argument" should {
      "find main function with data flow between getenv and strcpy for a x86_64 binary" in {
        buildCpgForBin("buf2.exe")
        val results = methodNamesForMatchedPoints(query)
        results shouldBe Set("main")
      }

      "find main function with data flow between getenv and strcpy for a MIPS binary" in {
        buildCpgForBin("buf2.mips.exe")
        val results = methodNamesForMatchedPoints(query)
        results shouldBe Set("main")
      }
    }
